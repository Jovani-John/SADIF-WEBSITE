...
const ProjectSkeleton = ({ layout }: { layout: any }) => (
  <div className={`${layout.span} col-span-12`}>
    <div className={`${layout.height} bg-gradient-to-br from-gray-200 to-gray-300 rounded-2xl animate-pulse`}>
      <div className="h-full p-6 flex flex-col justify-end">
        <div className="w-24 h-6 bg-gray-400/50 rounded mb-3" />
        <div className="w-3/4 h-8 bg-gray-400/50 rounded mb-2" />
        <div className="w-1/2 h-4 bg-gray-400/50 rounded" />
      </div>
    </div>
  </div>
);
...
export default function ProjectsPage() {
  ...
  // Memoize layout patterns
  const getProjectLayout = useCallback((index: number) => {
    const rowPatterns = [
    ...
    return rowPatterns[rowIndex % rowPatterns.length][positionInRow];
  }, []);
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
            <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.5 }}>
              ...
              <motion.div initial={{ width: 0 }} animate={{ width: "80px" }} transition={{ delay: 0.2, duration: 0.4 }} className="h-1 bg-[#979188] mt-3 sm:mt-4" />
            </motion.div>
          ...
          {filteredProjects.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 max-w-7xl mx-auto">
              {filteredProjects.map((project, index) => {
                ...
                const isPriority = index < 4; // First 4 are priority

                return (
                  <motion.div
                    key={project.id}
                    initial={{ y: 30, opacity: 0, x: isEven ? -30 : 30 }}
                  ...
                  >
                    <Link href={`/projects/${project.id}`} scroll={false} prefetch={isPriority}>
                      <motion.div
                        ...
                        transition={{ duration: 0.2 }}
                        className={`group relative ${layout.height} overflow-hidden cursor-pointer rounded-2xl`}
                      >
                        {/* Project Image */}
                        ...
                        <div className="absolute inset-0 p-4 sm:p-6 md:p-8 flex flex-col justify-end">
                          ...
                          <div className="h-0.5 w-0 bg-[#979188] mt-3 sm:mt-4 group-hover:w-16 transition-all duration-300" />
                        </div>
                      </motion.div>
                    </Link>
                  </motion.div>
